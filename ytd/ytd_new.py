...
#        "format":"bestaudio+bestvideo/best",
#        "external_downloader":"aria2c",
#        "external_downloader_args":[f"-x{aria_conns}",f"-s{aria_conns}","-k1M"],
#        "outtmpl":'%(title)s.%(ext)s',
#        "quiet":True,
#        "no_warnings":True
#    }
...
if __name__ == "__main__":
    ...
    x = input("Audio/Video (a/v): ")
    mode = "video"
    if x.strip() == 'a':
        mode = "audio"
        print("Selected mode = Audio/best")
    else:
        print("Selected mode = Video/best")
    print(mode)
    url_list=[]
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_downloads) as executor:
            results = executor.map(download_from,url_list,repeat(mode))
